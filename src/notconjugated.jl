...
function CbFFnotconj!(response::Array{TC}, K::AbstractArray, M::AbstractArray, 
      ...
      dim::Integer, n_excitedDOF::Integer, times::Ts, n_times::Integer, input_vectors::Matrix{TF},
    ...
    X_3 = Array{TC}(undef, dim, n_excitedDOF)
    ...
    # gamma2
    Γ_2 = zeros(TC, dim, n_excitedDOF)

    # Initializes the vectors of sums chi
    ...
    for j=1:n_excitedDOF
        ...
        # Initializes the gamma matrices
        Γ_1[:,j] = @. ((a1*X_5[:,j])-(((a1*times[1])+a0)*X_4[:,j])+C2)
        Γ_2[:,j] = @. ((a1*(X_2[:,j]+X_3[:,j]-X_5[:,j]))+(((a1*times[1])+a0)*(X_4[:,j]-X_1[:,j]))+C1)
        ...
        @. response[:,1] = (response[:,1]+(((times[1]*X_1[:,j])-X_2[:,j]-X_3[:,j])*χ_1[j])+
                            (χ_0[j]*X_1[:,j])+Γ_1[:,j]+Γ_2[:,j])

    end # j

    # Pre-alocate gamma_mult1j and gamma_mult2j
    Γ_mult1j = Vector{TC}(undef,dim)
    Γ_mult2j = Vector{TC}(undef,dim)
    ...
    for i=2:n_times
        ...
        for j=1:n_excitedDOF
            ...
            Γ_mult2j .= expCF_delta*Γ_2[:,j]

            # Calculates the response and adds the homogeneous response
            @. response[:,i] = (response[:,i]+Γ_mult1j+Γ_mult2j+(((t_current*X_1[:,j])-X_2[:,j]-X_3[:,j])*χ_1[j])+
                                (χ_0[j]*X_1[:,j])+C1+C2)

            # Updates the gamma vector
            Γ_1[:,j] = @. (Γ_mult1j+(cji1*X_5[:,j])-(((cji1*t_current)+cji0)*X_4[:,j]))
            Γ_2[:,j] = @. (Γ_mult2j+(cji1*(X_2[:,j]+X_3[:,j]-X_5[:,j]))+(((cji1*t_current)+cji0)*(X_4[:,j]-X_1[:,j])))
